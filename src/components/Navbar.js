  ...
  return (
    <nav className="bg-slate-900 shadow-xl">
			<div className="max-w-6xl mx-auto px-4">
					...
					<div className="flex space-x-7">
						<div>
							...
							<Link to="/" className="flex items-center py-4 px-2">
								...
								<span className="font-semibold text-gray-500 text-lg">Egle-Dashbord</span>
							</Link>
						</div>
						{/* <!-- Primary Navbar items --> */}
						<div className="hidden md:flex items-center space-x-1">
							<Link to="/" className={`py-4 px-2 text-green-500  border-green-500 font-semibold ${pathMatchRoute('/')?'border-b-4':null}`}>Home</Link>
							<Link to="/products" className={`block text-sm px-2 py-4  transition duration-300 ${pathMatchRoute('/products')?'border-b-4':null} `}>Products</Link>
							<Link to="/addProduct" className={`block text-sm px-2 py-4  transition duration-300 ${pathMatchRoute('/addProduct')?'border-b-4':null} `}>Add Product</Link>
							<Link to="/your-product" className={`block text-sm px-2 py-4  transition duration-300 ${pathMatchRoute('/your-product')?'border-b-4':null} `}>Your Products</Link>
							<Link to="/updateProduct" className={`block text-sm px-2 py-4  transition duration-300 ${pathMatchRoute('/updateProduct')?'border-b-4':null} `}>Update Products</Link>
						</div>
					</div>
              ...
              <Link to="/login" onClick={handleLogOut} className="py-2 px-2 font-medium text-gray-500 rounded hover:bg-green-500 hover:text-white transition duration-300">Log Out</Link>
              </>
            ):(
              <>
              <Link to="/login" className="py-2 px-2 font-medium text-gray-500 rounded hover:bg-green-500 hover:text-white transition duration-300">Log In</Link>
					...
					<div className="md:hidden flex items-center">
						<button className="outline-none mobile-menu-button" onClick={handleMobileView}>
